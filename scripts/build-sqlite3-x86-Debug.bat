...
call cmake -G "Visual Studio 17 2022" -S "%SolutionDir%thirdparty\sqlite3-cmake" -B "%SolutionDir%thirdparty\build\sqlite3\x86\Debug" -A Win32 -DCMAKE_INSTALL_PREFIX="%SolutionDir%thirdparty\install\sqlite3\x86\Debug" -DCMAKE_TOOLCHAIN_FILE="%SolutionDir%thirdparty\MetaHookSv\tools\toolchain.cmake" -Dsqlite3_BUILD_SHARED_LIBS=TRUE

call cmake --build "%SolutionDir%thirdparty\build\sqlite3\x86\Debug" --config Debug --target install
